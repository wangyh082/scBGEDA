...
load(['/data/',dataName,'_seed_',num2str(random_seed(seed_index)),'_',num2str(hvgs),'_',num2str(alpha),'.mat'])
...
for runIdx = 1:cntTimes
    ...
    % Or you can set up parameters by yourself.
     ...
     bcsLowK = 2;
     bcsUpK = 60;
     Label = BGEC(fea, k, m, distance, KNN, bcsLowK, bcsUpK);
...
disp('--------------------------------------------------------------');
disp(['mean NMI score: ',num2str(max(nmiScores))]);
disp(['mean ARI score: ',num2str(max(ariScores))]);
disp('--------------------------------------------------------------');
disp('**************************************************************');
